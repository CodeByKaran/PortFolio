...
let amount=document.getElementById("enteramount")
let from=document.getElementsByClassName("select");
let converting=document.getElementById("show")

currency_list.map((e)=>{
  let option=document.createElement('option')
  option.innerHTML=`${e.code}-${e.name}`
  from[0].insertAdjacentElement("beforeend",option);
  ...
  let option=document.createElement('option')
  option.innerHTML=`${e.code}-${e.name}`
  from[1].insertAdjacentElement("beforeend",option);
  ...
  fetch("https://v6.exchangerate-api.com/v6/2f0d246c6d3b10d65fd4e594/latest/USD")
  .then((res)=>res.json())
  .then((result)=>{
    
    converting.value=(amount.value/result.conversion_rates[from[0].value.slice(0,3)]* result.conversion_rates[from[1].value.slice(0,3)]).toFixed(2);
